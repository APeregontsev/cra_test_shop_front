...
const ShopingCart = () => {
  ...
  const isRegistrationFields = inputsData.address && inputsData.email && inputsData.name && inputsData.phone;
  ...
  let finalPrice =
    couponObtained && couponObtained !== "error" ? totalPrice - totalPrice * couponObtained : totalPrice;
      ...
      <div className="card-wrapper">
        ...
        <div className="products-data-wrapper">
          ...
          {Object.keys(shopingCard).map((item) => {
            const details = productsInCard[item];
            return (
              details && <ShopingCard amount={shopingCard[item]} details={productsInCard[item]} key={item} />
            );
          })}
        </div>
      </div>
